 ...
 * {@link org.lealone.hansql.optimizer.rel.logical.LogicalCalc} with a program for which
...
public class ReduceDecimalsRule extends RelOptRule {
    public static final ReduceDecimalsRule INSTANCE = new ReduceDecimalsRule(RelFactories.LOGICAL_BUILDER);
    ...
    private class ExpanderMap {
        ...
        private void registerExpanders(RexBuilder rexBuilder) {
            ...
            map.put(SqlStdOperatorTable.GREATER_THAN_OR_EQUAL, arithmetic);
    ...
    public abstract class RexExpander {
        ...
        protected RexNode scaleUp(RexNode value, int scale) {
            ...
            return builder.makeCall(SqlStdOperatorTable.MULTIPLY, value, makeScaleFactor(scale));
        }
        ...
        protected RexNode scaleDown(RexNode value, int scale) {
            ...
            if (scale == maxPrecision) {
                ...
                return makeCase(
                        builder.makeCall(SqlStdOperatorTable.GREATER_THAN_OR_EQUAL, value, makeExactLiteral(half)),
                        makeExactLiteral(1),
                        builder.makeCall(SqlStdOperatorTable.LESS_THAN_OR_EQUAL, value, makeExactLiteral(-half)),
                        makeExactLiteral(-1), makeExactLiteral(0));
            }
            RexNode roundFactor = makeRoundFactor(scale);
            RexNode roundValue = makeCase(
                    builder.makeCall(SqlStdOperatorTable.GREATER_THAN, value, makeExactLiteral(0)),
                    makePlus(value, roundFactor), makeMinus(value, roundFactor));
            return makeDivide(roundValue, makeScaleFactor(scale));
        }
        ...
        protected RexNode ensureScale(RexNode value, int scale, int required) {
            final RelDataTypeSystem typeSystem = builder.getTypeFactory().getTypeSystem();
            ...
            if (scaleDiff >= maxPrecision) {
                throw Util.needToImplement(
                        "Source type with scale " + scale + " cannot be converted to target type with scale " + required
                                + " because the smallest value of the "
                                + "source type is too large to be encoded by the " + "target type");
            }
        ...
        protected RexNode makeCase(RexNode condition, RexNode thenClause, RexNode elseClause) {
            return builder.makeCall(SqlStdOperatorTable.CASE, condition, thenClause, elseClause);
        }

        protected RexNode makeCase(RexNode whenA, RexNode thenA, RexNode whenB, RexNode thenB, RexNode elseClause) {
            return builder.makeCall(SqlStdOperatorTable.CASE, whenA, thenA, whenB, thenB, elseClause);
        }

        protected RexNode makePlus(RexNode a, RexNode b) {
            return builder.makeCall(SqlStdOperatorTable.PLUS, a, b);
        }

        protected RexNode makeMinus(RexNode a, RexNode b) {
            return builder.makeCall(SqlStdOperatorTable.MINUS, a, b);
        }

        protected RexNode makeDivide(RexNode a, RexNode b) {
            return builder.makeCall(SqlStdOperatorTable.DIVIDE_INTEGER, a, b);
        }

        protected RexNode makeMultiply(RexNode a, RexNode b) {
            return builder.makeCall(SqlStdOperatorTable.MULTIPLY, a, b);
        }

        protected RexNode makeIsPositive(RexNode a) {
            return builder.makeCall(SqlStdOperatorTable.GREATER_THAN, a, makeExactLiteral(0));
        }

        protected RexNode makeIsNegative(RexNode a) {
            return builder.makeCall(SqlStdOperatorTable.LESS_THAN, a, makeExactLiteral(0));
        }
    }
    ...
    private class BinaryArithmeticExpander extends RexExpander {
        ...
        private RexNode expandComparison(RexCall call, List<RexNode> operands) {
            ...
            return builder.makeCall(call.getOperator(), ensureScale(accessValue(operands.get(0)), scaleA, commonScale),
                    ensureScale(accessValue(operands.get(1)), scaleB, commonScale));
        }
        ...
        private RexNode expandMod(RexCall call, List<RexNode> operands) {
            ...
            RexNode result = builder.makeCall(call.getOperator(), accessValue(operands.get(0)),
    ...
    private class FloorExpander extends RexExpander {
        ...
        public RexNode expand(RexCall call) {
            ...
            RexNode decValue = call.getOperands().get(0);
            ...
            if (scale == 0) {
                ...
                rewrite = makeCase(makeIsNegative(value), makeExactLiteral(-1), makeExactLiteral(0));
            } else {
                RexNode round = makeExactLiteral(1 - powerOfTen(scale));
                RexNode scaleFactor = makeScaleFactor(scale);
                rewrite = makeCase(makeIsNegative(value), makeDivide(makePlus(value, round), scaleFactor),
    ...
    private class CeilExpander extends RexExpander {
        ...
        public RexNode expand(RexCall call) {
            ...
            if (scale == 0) {
                ...
                rewrite = makeCase(makeIsPositive(value), makeExactLiteral(1), makeExactLiteral(0));
            } else {
                RexNode round = makeExactLiteral(powerOfTen(scale) - 1);
                RexNode scaleFactor = makeScaleFactor(scale);
                rewrite = makeCase(makeIsPositive(value), makeDivide(makePlus(value, round), scaleFactor),
                        makeDivide(value, scaleFactor));
            }
            return encodeValue(rewrite, call.getType());
        }
    ...
    private class CastArgAsDoubleExpander extends CastArgAsTypeExpander {
        ...
        public RelDataType getArgType(RexCall call, int ordinal) {
            ...
            if (call.getOperands().get(ordinal).getType().isNullable()) {
    ...
    private class ReinterpretExpander extends RexExpander {
        ...
        @Override
        public boolean canExpand(RexCall call) {
            return call.isA(SqlKind.REINTERPRET) && call.getOperands().get(0).isA(SqlKind.REINTERPRET);
        }
        ...
        public RexNode expand(RexCall call) {
            ...
            RexNode innerValue = subCall.getOperands().get(0);
