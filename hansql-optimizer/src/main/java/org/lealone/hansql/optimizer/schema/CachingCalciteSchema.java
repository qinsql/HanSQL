...
import org.lealone.hansql.optimizer.util.NameMap;
import org.lealone.hansql.optimizer.util.NameMultimap;
import org.lealone.hansql.optimizer.util.NameSet;
...
class CachingCalciteSchema extends CalciteSchema {
    ...
    private CachingCalciteSchema(CalciteSchema parent, Schema schema, String name, NameMap<CalciteSchema> subSchemaMap,
            NameMap<TableEntry> tableMap, NameMap<TypeEntry> typeMap, NameMultimap<FunctionEntry> functionMap,
            NameSet functionNames, NameMap<FunctionEntry> nullaryFunctionMap, List<? extends List<String>> path) {
        super(parent, schema, name, subSchemaMap, tableMap, typeMap, functionMap, functionNames, nullaryFunctionMap,
                path);
    ...
    protected CalciteSchema snapshot(CalciteSchema parent, SchemaVersion version) {
        CalciteSchema snapshot = new CachingCalciteSchema(parent, schema.snapshot(version), name, null, tableMap,
                typeMap, functionMap, functionNames, nullaryFunctionMap, getPath());
        ...
        private SubSchemaCache(final CalciteSchema calciteSchema, Set<String> names) {
            ...
            this.cache = CacheBuilder.newBuilder().build(new CacheLoader<String, CalciteSchema>() {
                ...
                public CalciteSchema load(String schemaName) {
                    final Schema subSchema = calciteSchema.schema.getSubSchema(schemaName);
                    if (subSchema == null) {
