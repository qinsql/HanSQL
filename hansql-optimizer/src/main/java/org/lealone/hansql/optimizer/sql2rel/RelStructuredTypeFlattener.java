...
import org.lealone.hansql.optimizer.util.ReflectiveVisitDispatcher;
...
public class RelStructuredTypeFlattener implements ReflectiveVisitor {
    ...
    public RelStructuredTypeFlattener(RexBuilder rexBuilder, RelOptTable.ToRelContext toRelContext,
            boolean restructure) {
        this(RelFactories.LOGICAL_BUILDER.create(toRelContext.getCluster(), null), rexBuilder, toRelContext,
                restructure);
    }

    public RelStructuredTypeFlattener(RelBuilder relBuilder, RexBuilder rexBuilder,
            RelOptTable.ToRelContext toRelContext, boolean restructure) {
    ...
    public RelNode rewrite(RelNode root) {
        ...
        if (restructured) {
            ...
            // structured types.
            return relBuilder.push(flattened).projectNamed(structuringExps, root.getRowType().getFieldNames(), true)
                    .build();
        } else {
            return flattened;
        }
    ...
    private void flattenProjections(RewriteRexShuttle shuttle, List<? extends RexNode> exps, List<String> fieldNames,
            String prefix, List<Pair<RexNode, String>> flattenedExps) {
    ...
    private void flattenProjection(RewriteRexShuttle shuttle, RexNode exp, String fieldName,
        ...
        if (exp.getType().isStruct()) {
            if (exp instanceof RexInputRef) {
                ...
                // expand to range
                RelDataType flattenedType = SqlTypeUtil.flattenRecordType(rexBuilder.getTypeFactory(), exp.getType(),
                        null);
                ...
                for (int j = 0; j < n; ++j) {
                    ...
                    flattenedExps.add(
                            Pair.of(RexBuilder.getRexFactory().makeInputRef(newField.i + j, newField.e), fieldName));
                }
                ...
                flattenProjections(new RewriteRexShuttle(), call.getOperands(),
                        Collections.nCopies(call.getOperands().size(), null), fieldName, flattenedExps);
                ...
                for (RelDataTypeField field : newExp.getType().getFieldList()) {
                    flattenedExps.add(
                            Pair.of(rexBuilder.makeFieldAccess(newExp, field.getIndex()), fieldName + "$" + (j++)));
                }
            } else {
    ...
    private class RewriteRelVisitor extends RelVisitor {
        private final ReflectiveVisitDispatcher<RelStructuredTypeFlattener, RelNode> dispatcher = ReflectUtil
                .createDispatcher(RelStructuredTypeFlattener.class, RelNode.class);
            ...
            if (!found) {
                if (p.getInputs().size() == 0) {
                    ...
                    throw new AssertionError(
                            "no '" + visitMethodName + "' method found for class " + p.getClass().getName());
                }
            }
        }
    }
    ...
    private class RewriteRexShuttle extends RexShuttle {
        ...
        public RexNode visitFieldAccess(RexFieldAccess fieldAccess) {
            ...
            for (;;) {
                ...
                if (refExp instanceof RexInputRef) {
                    ...
                    RelDataType refType = SqlTypeUtil.flattenRecordType(rexBuilder.getTypeFactory(), refExp.getType(),
                            null);
        ...
        public RexNode visitSubQuery(RexSubQuery subQuery) {
            ...
            RelStructuredTypeFlattener flattener = new RelStructuredTypeFlattener(relBuilder, rexBuilder, toRelContext,
                    restructure);
