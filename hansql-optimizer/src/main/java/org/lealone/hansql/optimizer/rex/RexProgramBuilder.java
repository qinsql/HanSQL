...
public class RexProgramBuilder {
     ...
     */
    private RexProgramBuilder(RexBuilder rexBuilder, final RelDataType inputRowType, final List<RexNode> exprList,
            final Iterable<? extends RexNode> projectList, RexNode condition, final RelDataType outputRowType,
            boolean normalize, RexSimplify simplify) {
        ...
        if (condition != null) {
            if (simplify != null) {
                condition = simplify
                        .simplify(rexBuilder.makeCall(SqlStdOperatorTable.IS_TRUE, condition.accept(expander)));
    ...
    public RexLocalRef addExpr(RexNode expr) {
        ...
        exprList.add(expr);
        ref = RexBuilder.getRexFactory().makeLocalRef(index, expr.getType());
    ...
    public RexProgram getProgram(boolean normalize) {
        ...
        if (normalize) {
            return create(rexBuilder, inputRowType, exprList, projectRefList, conditionRef, outputRowType, true)
                    .getProgram(false);
        }

        return new RexProgram(inputRowType, exprList, projectRefList, conditionRef, outputRowType);
    }
    ...
    public static RexProgramBuilder forProgram(RexProgram program, RexBuilder rexBuilder, boolean normalize) {
        ...
        return create(rexBuilder, inputRowType, exprs, projectRefs, conditionRef, outputRowType, normalize, false);
    }
    ...
    public static RexProgramBuilder create(RexBuilder rexBuilder, final RelDataType inputRowType,
            ...
            final RelDataType outputRowType, boolean normalize, RexSimplify simplify) {
    ...
    public static RexProgramBuilder create(RexBuilder rexBuilder, final RelDataType inputRowType,
            final List<RexNode> exprList, final List<? extends RexNode> projectList, final RexNode condition,
            final RelDataType outputRowType, boolean normalize) {
     ...
     */
    public static RexProgramBuilder create(RexBuilder rexBuilder, final RelDataType inputRowType,
            final List<RexNode> exprList, final List<RexLocalRef> projectRefList, final RexLocalRef conditionRef,
            final RelDataType outputRowType, final RexShuttle shuttle, final boolean updateRefs) {
     ...
     *                       of rewrites made by the shuttle
     */
    private void add(List<RexNode> exprList, List<RexLocalRef> projectRefList, RexLocalRef conditionRef,
            final RelDataType outputRowType, RexShuttle shuttle, boolean updateRefs) {
    ...
    public void addIdentity() {
        ...
        for (RelDataTypeField field : inputRowType.getFieldList()) {
            addProject(RexBuilder.getRexFactory().makeInputRef(field.getIndex(), field.getType()), field.getName());
        }
    }
    ...
    public RexLocalRef makeInputRef(int index) {
        ...
        return RexBuilder.getRexFactory().makeLocalRef(index, field.getType());
    }
    ...
    private class RegisterInputShuttle extends RegisterShuttle {
        ...
        public RexNode visitInputRef(RexInputRef input) {
            ...
            if (valid) {
                ...
                if ((index < 0) || (index >= inputRowType.getFieldCount())) {
                    assert false : "RexInputRef index " + index + " out of range 0.."
                ...
                // skip the check.
                assert input.getType().isStruct() || RelOptUtil.eq("type1", input.getType(), "type2",
                        inputRowType.getFieldList().get(index).getType(), Litmus.THROW);
            }
        ...
        public RexNode visitLocalRef(RexLocalRef local) {
            if (valid) {
                ...
                assert RelOptUtil.eq("expr type", exprList.get(index).getType(), "ref type", local.getType(),
                        Litmus.THROW);
            }
            ...
            while (true) {
                ...
                if (expr instanceof RexLocalRef) {
                    ...
                    if (local.getIndex() >= index) {
                        throw new AssertionError("expr " + local + " references later expr " + local.getIndex());
                    }
                } else {
