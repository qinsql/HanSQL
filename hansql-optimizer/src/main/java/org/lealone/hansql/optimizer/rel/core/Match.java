...
import org.lealone.hansql.optimizer.sql.fun.SqlBitOpAggFunction;
import org.lealone.hansql.optimizer.sql.fun.SqlMinMaxAggFunction;
import org.lealone.hansql.optimizer.sql.fun.SqlStdOperatorTable;
import org.lealone.hansql.optimizer.sql.fun.SqlSumAggFunction;
...
public abstract class Match extends SingleRel {
     ...
     */
    protected Match(RelOptCluster cluster, RelTraitSet traitSet, RelNode input, RelDataType rowType, RexNode pattern,
            boolean strictStart, boolean strictEnd, Map<String, RexNode> patternDefinitions,
            Map<String, RexNode> measures, RexNode after, Map<String, ? extends SortedSet<String>> subsets,
            boolean allRows, List<RexNode> partitionKeys, RelCollation orderKeys, RexNode interval) {
    ...
    public abstract Match copy(RelNode input, RelDataType rowType, RexNode pattern, boolean strictStart,
            boolean strictEnd, Map<String, RexNode> patternDefinitions, Map<String, RexNode> measures, RexNode after,
            Map<String, ? extends SortedSet<String>> subsets, boolean allRows, List<RexNode> partitionKeys,
            RelCollation orderKeys, RexNode interval);
    ...
    public RelNode copy(RelTraitSet traitSet, List<RelNode> inputs) {
        ...
        return copy(inputs.get(0), rowType, pattern, strictStart, strictEnd, patternDefinitions, measures, after,
                subsets, allRows, partitionKeys, orderKeys, interval);
    }

    @Override
    public RelWriter explainTerms(RelWriter pw) {
        return super.explainTerms(pw).item("partition", getPartitionKeys()).item("order", getOrderKeys())
                .item("outputFields", getRowType().getFieldNames()).item("allRows", isAllRows())
                .item("after", getAfter()).item("pattern", getPattern()).item("isStrictStarts", isStrictStart())
                .item("isStrictEnds", isStrictEnd()).itemIf("interval", getInterval(), getInterval() != null)
    ...
    private static class AggregateFinder extends RexVisitorImpl {
        ...
        public Object visitCall(RexCall call) {
            ...
            if (aggFunction != null) {
                RexMRAggCall aggCall = RexBuilder.getRexFactory().makeMRAggCall(aggFunction, call.getType(),
                        call.getOperands(), aggregateCalls.size());
     ...
     */
    public static interface RexMRAggCall extends RexCall, Comparable<RexMRAggCall> {
        int getOrdinal();
    }
}
