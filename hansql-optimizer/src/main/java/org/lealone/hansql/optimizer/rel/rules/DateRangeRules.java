...
import org.lealone.hansql.optimizer.util.DateTimeUtils;
...
import org.lealone.hansql.optimizer.util.TimestampWithTimeZoneString;
...
public abstract class DateRangeRules {
    ...
    private static final Predicate<Filter> FILTER_PREDICATE = filter -> {
        try (ExtractFinder finder = ExtractFinder.THREAD_INSTANCES.get()) {
            ...
            return finder.timeUnits.contains(TimeUnitRange.YEAR) || finder.opKinds.contains(SqlKind.FLOOR)
    ...
    private static final Map<TimeUnitRange, Integer> TIME_UNIT_CODES = ImmutableMap.<TimeUnitRange, Integer> builder()
            .put(TimeUnitRange.YEAR, Calendar.YEAR).put(TimeUnitRange.MONTH, Calendar.MONTH)
            .put(TimeUnitRange.DAY, Calendar.DAY_OF_MONTH).put(TimeUnitRange.HOUR, Calendar.HOUR)
            .put(TimeUnitRange.MINUTE, Calendar.MINUTE).put(TimeUnitRange.SECOND, Calendar.SECOND)
            .put(TimeUnitRange.MILLISECOND, Calendar.MILLISECOND).build();

    private static final Map<TimeUnitRange, TimeUnitRange> TIME_UNIT_PARENTS = ImmutableMap
            .<TimeUnitRange, TimeUnitRange> builder().put(TimeUnitRange.MONTH, TimeUnitRange.YEAR)
            .put(TimeUnitRange.DAY, TimeUnitRange.MONTH).put(TimeUnitRange.HOUR, TimeUnitRange.DAY)
            .put(TimeUnitRange.MINUTE, TimeUnitRange.HOUR).put(TimeUnitRange.SECOND, TimeUnitRange.MINUTE)
            .put(TimeUnitRange.MILLISECOND, TimeUnitRange.SECOND).put(TimeUnitRange.MICROSECOND, TimeUnitRange.SECOND)
            .build();
    ...
    public static class FilterDateRangeRule extends RelOptRule {
        public FilterDateRangeRule(RelBuilderFactory relBuilderFactory) {
            super(operandJ(Filter.class, null, FILTER_PREDICATE, any()), relBuilderFactory, "FilterDateRangeRule");
        }
            ...
            case LESS_THAN:
                ...
                case LITERAL:
                    ...
                    if (isFloorCeilCall(op1)) {
                        assert op1 instanceof RexCall;
                        final RexCall subCall = (RexCall) op1;
                        final RexLiteral flag = (RexLiteral) subCall.getOperands().get(1);
                        final TimeUnitRange timeUnit = (TimeUnitRange) flag.getValue();
                        return compareFloorCeil(call.getKind().reverse(), subCall.getOperands().get(0),
                                (RexLiteral) op0, timeUnit, op1.getKind() == SqlKind.FLOOR);
                    }
                }
                switch (op1.getKind()) {
                case LITERAL:
                    ...
                    if (isExtractCall(op0)) {
                        ...
                        RexNode operand = subCall.getOperands().get(1);
                        if (canRewriteExtract(operand)) {
                            return compareExtract(call.getKind(), subCall.getOperands().get(1), (RexLiteral) op1);
                        }
                    }
                    if (isFloorCeilCall(op0)) {
                        ...
                        final RexLiteral flag = (RexLiteral) subCall.getOperands().get(1);
                        final TimeUnitRange timeUnit = (TimeUnitRange) flag.getValue();
                        return compareFloorCeil(call.getKind(), subCall.getOperands().get(0), (RexLiteral) op1,
                ...
                for (RexNode operand : exprs) {
                    ...
                    for (TimeUnitRange timeUnit : timeUnitRanges) {
                        clonedOperand = clonedOperand.accept(
                                new ExtractShuttle(rexBuilder, timeUnit, operandRanges, timeUnitRanges, timeZone));
                    }
