...
import org.lealone.hansql.optimizer.rel.logical.LogicalWindow;
 ...
 * a {@link org.lealone.hansql.optimizer.rel.logical.LogicalProject}
 * past a {@link org.lealone.hansql.optimizer.rel.logical.LogicalWindow}.
 */
public class ProjectWindowTransposeRule extends RelOptRule {
    /** The default instance of
     * {@link org.lealone.hansql.optimizer.rel.rules.ProjectWindowTransposeRule}. */
    ...
    public void onMatch(RelOptRuleCall call) {
        ...
        for (int index : BitSets.toIter(beReferred)) {
            ...
            exps.add(RexBuilder.getRexFactory().makeInputRef(index, relDataTypeField.getType()));
            builder.add(relDataTypeField);
        }

        final LogicalProject projectBelowWindow = new LogicalProject(cluster, window.getTraitSet(), window.getInput(),
                exps, builder.build());
        ...
        final RexShuttle indexAdjustment = new RexShuttle() {
            ...
            public RexNode visitInputRef(RexInputRef inputRef) {
                ...
                return RexBuilder.getRexFactory().makeInputRef(newIndex, inputRef.getType());
            }
            ...
            public RexNode visitCall(final RexCall call) {
                if (call instanceof Window.RexWinAggCall) {
                    ...
                    if (update[0]) {
                        return RexBuilder.getRexFactory().makeWinAggCall((SqlAggFunction) call.getOperator(),
                                call.getType(), clonedOperands, ((Window.RexWinAggCall) call).getOrdinal(),
                                ((Window.RexWinAggCall) call).isDistinct());
                    } else {
                        return call;
                    }
                } else {
        ...
        for (Window.Group group : window.groups) {
            ...
            groups.add(new Window.Group(keys.build(), group.isRows, group.lowerBound, group.upperBound,
                    RelCollations.of(orderKeys), aggCalls));
        }

        final LogicalWindow newLogicalWindow = LogicalWindow.create(window.getTraitSet(), projectBelowWindow,
        ...
        final LogicalProject newTopProj = project.copy(newLogicalWindow.getTraitSet(), newLogicalWindow, topProjExps,
                project.getRowType());
