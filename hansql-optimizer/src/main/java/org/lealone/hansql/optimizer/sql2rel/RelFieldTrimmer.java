...
import org.lealone.hansql.optimizer.rex.RexPermuteInputsShuttle;
...
import org.lealone.hansql.optimizer.util.mapping.IntPair;
...
public class RelFieldTrimmer implements ReflectiveVisitor {
     ...
     * @return New relational expression and its field mapping
     */
    protected final TrimResult dispatchTrimFields(RelNode rel, ImmutableBitSet fieldsUsed,
            Set<RelDataTypeField> extraFields) {
        ...
        assert mapping.getTargetCount() + extraFields.size() == newFieldCount || Bug.TODO_FIXED : "target: "
    ...
    /**
     * Visit method, per {@link org.lealone.hansql.optimizer.util.ReflectiveVisitor}.
     *
    ...
    /**
     * Variant of {@link #trimFields(RelNode, ImmutableBitSet, Set)} for
     * {@link org.lealone.hansql.optimizer.rel.core.Sort}.
     */
    ...
    public TrimResult trimFields(Join join, ImmutableBitSet fieldsUsed, Set<RelDataTypeField> extraFields) {
        final int fieldCount = join.getSystemFieldList().size() + join.getLeft().getRowType().getFieldCount()
        ...
        for (RelNode input : join.getInputs()) {
            ...
            // on-demand fields.
            Set<RelDataTypeField> inputExtraFields = RelDataTypeImpl.extra(inputRowType) == null
        ...
        if (join instanceof SemiJoin) {
            ...
            mapping = Mappings.create(MappingType.INVERSE_SURJECTION, join.getRowType().getFieldCount(),
    ...
    /**
     * Variant of {@link #trimFields(RelNode, ImmutableBitSet, Set)} for
     * {@link org.lealone.hansql.optimizer.rel.core.SetOp} (including UNION and UNION ALL).
     */
    ...
    /**
     * Variant of {@link #trimFields(RelNode, ImmutableBitSet, Set)} for
     * {@link org.lealone.hansql.optimizer.rel.logical.LogicalAggregate}.
     */
    public TrimResult trimFields(Aggregate aggregate, ImmutableBitSet fieldsUsed, Set<RelDataTypeField> extraFields) {
        ...
        Mapping mapping = Mappings.create(MappingType.INVERSE_SURJECTION, rowType.getFieldCount(),
        ...
        final ImmutableList<ImmutableBitSet> newGroupSets = ImmutableList
                .copyOf(Iterables.transform(aggregate.getGroupSets(), input1 -> Mappings.apply(inputMapping, input1)));
        ...
        for (AggregateCall aggCall : aggregate.getAggCallList()) {
            if (fieldsUsed.get(j)) {
                final ImmutableList<RexNode> args = relBuilder
                        .fields(Mappings.apply2(inputMapping, aggCall.getArgList()));
                final RexNode filterArg = aggCall.filterArg < 0 ? null
                        : relBuilder.field(Mappings.apply(inputMapping, aggCall.filterArg));
                RelBuilder.AggCall newAggCall = relBuilder.aggregateCall(aggCall.getAggregation(), args)
                        .distinct(aggCall.isDistinct()).filter(filterArg).approximate(aggCall.isApproximate())
                        .sort(relBuilder.fields(aggCall.collation)).as(aggCall.name);
    ...
    /**
     * Variant of {@link #trimFields(RelNode, ImmutableBitSet, Set)} for
     * {@link org.lealone.hansql.optimizer.rel.logical.LogicalTableModify}.
     */
    ...
    /**
     * Variant of {@link #trimFields(RelNode, ImmutableBitSet, Set)} for
     * {@link org.lealone.hansql.optimizer.rel.logical.LogicalTableFunctionScan}.
     */
    public TrimResult trimFields(LogicalTableFunctionScan tabFun, ImmutableBitSet fieldsUsed,
            Set<RelDataTypeField> extraFields) {
        ...
        if (!tabFun.getInputs().equals(newInputs)) {
            newTabFun = tabFun.copy(tabFun.getTraitSet(), newInputs, tabFun.getCall(), tabFun.getElementType(),
                    tabFun.getRowType(), tabFun.getColumnMappings());
        }
    ...
    /**
     * Variant of {@link #trimFields(RelNode, ImmutableBitSet, Set)} for
     * {@link org.lealone.hansql.optimizer.rel.logical.LogicalValues}.
     */
    ...
    /**
     * Variant of {@link #trimFields(RelNode, ImmutableBitSet, Set)} for
     * {@link org.lealone.hansql.optimizer.rel.logical.LogicalTableScan}.
     */
     ...
     *
     * <p>The mapping is a
     * {@link org.lealone.hansql.optimizer.util.mapping.Mappings.SourceMapping}, which means
    ...
    protected static class TrimResult extends Pair<RelNode, Mapping> {
        ...
        public TrimResult(RelNode left, Mapping right) {
            super(left, right);
            assert right.getTargetCount() == left.getRowType().getFieldCount() : "rowType: " + left.getRowType()
                    + ", mapping: " + right;
        }
    }
}
