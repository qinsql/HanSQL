...
/**
 * Sub-class of {@link org.lealone.hansql.optimizer.rel.core.Window}
...
public final class LogicalWindow extends Window {
     ...
     */
    public LogicalWindow(RelOptCluster cluster, RelTraitSet traitSet, RelNode input, List<RexLiteral> constants,
     ...
     * @param groups Window groups
     */
    public static LogicalWindow create(RelTraitSet traitSet, RelNode input, List<RexLiteral> constants,
            RelDataType rowType, List<Group> groups) {
     ...
     */
    public static RelNode create(RelOptCluster cluster, RelTraitSet traitSet, RelBuilder relBuilder, RelNode child,
        ...
        RexShuttle replaceConstants = new RexShuttle() {
            ...
            public RexNode visitLiteral(RexLiteral literal) {
                ...
                ref = RexBuilder.getRexFactory().makeInputRef(constantPool.size() + inputFieldCount, literal.getType());
        ...
        for (Map.Entry<WindowKey, Collection<RexOver>> entry : windowMap.asMap().entrySet()) {
            ...
            for (RexOver over : entry.getValue()) {
                final RexWinAggCall aggCall = RexBuilder.getRexFactory().makeWinAggCall(over.getAggOperator(),
                        over.getType(), toInputRefs(over.getOperands()), aggMap.size(), over.isDistinct());
            ...
            RexShuttle toInputRefs = new RexShuttle() {
                ...
                public RexNode visitLocalRef(RexLocalRef localRef) {
                    return RexBuilder.getRexFactory().makeInputRef(localRef.getIndex(), localRef.getType());
                }
            };
            groups.add(new Group(windowKey.groupSet, windowKey.isRows, windowKey.lowerBound.accept(toInputRefs),
                    windowKey.upperBound.accept(toInputRefs), windowKey.orderKeys, aggCalls));
        }
        ...
        final List<Map.Entry<String, RelDataType>> fieldList = new ArrayList<>(child.getRowType().getFieldList());
        ...
        RexShuttle shuttle = new RexShuttle() {
            ...
            public RexNode visitOver(RexOver over) {
                ...
                assert RelOptUtil.eq("over", over.getType(), "intermed",
                        intermediateRowType.getFieldList().get(index).getType(), Litmus.THROW);
                return RexBuilder.getRexFactory().makeInputRef(index, over.getType());
            }
            ...
            public RexNode visitLocalRef(RexLocalRef localRef) {
                ...
                return RexBuilder.getRexFactory().makeLocalRef(flattenedAggCallList.size() + index, localRef.getType());
            }
        };
    ...
    private static List<RexNode> toInputRefs(final List<? extends RexNode> operands) {
        return new AbstractList<RexNode>() {
            ...
            public RexNode get(int index) {
                ...
                return RexBuilder.getRexFactory().makeInputRef(ref.getIndex(), ref.getType());
            }
        };
    }
    ...
    private static class WindowKey {
        ...
        WindowKey(ImmutableBitSet groupSet, RelCollation orderKeys, boolean isRows, RexWindowBound lowerBound,
        ...
        public boolean equals(Object obj) {
            return obj == this || obj instanceof WindowKey && groupSet.equals(((WindowKey) obj).groupSet)
                    && orderKeys.equals(((WindowKey) obj).orderKeys)
                    && Objects.equals(lowerBound, ((WindowKey) obj).lowerBound)
                    && Objects.equals(upperBound, ((WindowKey) obj).upperBound) && isRows == ((WindowKey) obj).isRows;
        }
    }
