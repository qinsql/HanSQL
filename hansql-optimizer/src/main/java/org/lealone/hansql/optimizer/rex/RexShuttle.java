 ...
 * {@link org.lealone.hansql.optimizer.util.Glossary#VISITOR_PATTERN Visitor Pattern}. Use
...
public class RexShuttle implements RexVisitor<RexNode> {
    ...
    public RexNode visitOver(RexOver over) {
        ...
        List<RexNode> clonedOperands = visitList(over.getOperands(), update);
        ...
        if (update[0] || (window != over.getWindow())) {
            ...
            return RexBuilder.getRexFactory().makeOver(over.getType(), over.getAggOperator(), clonedOperands, window,
                    over.isDistinct());
        } else {
            return over;
        }
    ...
    public RexWindow visitWindow(RexWindow window) {
        ...
        if (update[0] || (lowerBound != window.getLowerBound() && lowerBound != null)
    ...
    public RexNode visitSubQuery(RexSubQuery subQuery) {
        boolean[] update = { false };
        List<RexNode> clonedOperands = visitList(subQuery.getOperands(), update);
        if (update[0]) {
    ...
    public RexNode visitFieldAccess(RexFieldAccess fieldAccess) {
        ...
        if (before == after) {
            ...
            return RexBuilder.getRexFactory().makeFieldAccess(after, fieldAccess.getField());
        }
    }
    ...
    public final Iterable<RexNode> apply(Iterable<? extends RexNode> iterable) {
        return Iterables.transform(iterable, t -> t == null ? null : t.accept(RexShuttle.this));
    }
