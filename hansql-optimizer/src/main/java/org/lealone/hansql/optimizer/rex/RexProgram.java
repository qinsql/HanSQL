...
public class RexProgram {
     ...
     */
    public RexProgram(RelDataType inputRowType, List<? extends RexNode> exprs, List<RexLocalRef> projects,
            RexLocalRef condition, RelDataType outputRowType) {
     ...
     * @return A program
     */
    public static RexProgram create(RelDataType inputRowType, List<? extends RexNode> projectExprs,
            RexNode conditionExpr, RelDataType outputRowType, RexBuilder rexBuilder) {
     ...
     */
    public static RexProgram create(RelDataType inputRowType, List<? extends RexNode> projectExprs,
            RexNode conditionExpr, List<String> fieldNames, RexBuilder rexBuilder) {
        if (fieldNames == null) {
            fieldNames = Collections.nCopies(projectExprs.size(), null);
        } else {
            assert fieldNames.size() == projectExprs.size() : "fieldNames=" + fieldNames + ", exprs=" + projectExprs;
        }
    ...
    public RelWriter collectExplainTerms(String prefix, RelWriter pw, SqlExplainLevel level) {
        ...
        assert outFields.size() == projects.size() : "outFields.length=" + outFields.size() + ", projects.length="
                + projects.size();
    ...
    public static RexProgram createIdentity(RelDataType rowType, RelDataType outputRowType) {
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            refs.add(ref);
            projectRefs.add(RexBuilder.getRexFactory().makeLocalRef(i, ref.getType()));
        }
    ...
    public boolean isNull(RexNode expr) {
        ...
        case LOCAL_REF:
            ...
            return isNull(exprs.get(inputRef.getIndex()));
        case CAST:
     ...
     */
    public static void deduceCollations(List<RelCollation> outputCollations, final int sourceCount,
            List<RexLocalRef> refs, List<RelCollation> inputCollations) {
        ...
        loop: for (RelCollation collation : inputCollations) {
    ...
    public boolean projectsIdentity(final boolean fail) {
        ...
        if (projects.size() < fieldCount) {
            assert !fail : "program '" + toString() + "' does not project identity for input row type '" + inputRowType
                    + "'";
            return false;
        }
        for (int i = 0; i < fieldCount; i++) {
            ...
            if (project.getIndex() != i) {
                assert !fail : "program " + toString() + "' does not project identity for input row type '"
    ...
    public int getSourceField(int outputOrdinal) {
        ...
        while (true) {
            ...
            if (expr instanceof RexLocalRef) {
                index = ((RexLocalRef) expr).getIndex();
    ...
    public RexProgram normalize(RexBuilder rexBuilder, RexSimplify simplify) {
        ...
        assert isValid(Litmus.THROW, null);
        final RexProgramBuilder builder = RexProgramBuilder.create(rexBuilder, inputRowType, exprs, projects, condition,
                outputRowType, true, simplify);
    ...
    private class ConstantFinder extends RexUtil.ConstantFinder {
        @Override
        public Boolean visitLocalRef(RexLocalRef localRef) {
            final RexNode expr = exprs.get(localRef.getIndex());
    ...
    private class Marshaller extends RexVisitorImpl<RexNode> {
        ...
        public RexNode visitFieldAccess(RexFieldAccess fieldAccess) {
            ...
            return RexBuilder.getRexFactory().makeFieldAccess(referenceExpr, fieldAccess.getField());
        }
    }
