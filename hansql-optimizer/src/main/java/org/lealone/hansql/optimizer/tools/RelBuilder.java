...
import org.lealone.hansql.optimizer.rel.core.Match;
...
import org.lealone.hansql.optimizer.rex.RexSimplify;
...
import org.lealone.hansql.optimizer.util.Holder;
...
public class RelBuilder {
    ...
    public String toString() {
        return stack.stream().map(frame -> RelOptUtil.toString(frame.rel)).collect(Collectors.joining(""));
    }
    ...
    private RexNode field(int inputCount, int inputOrdinal, int fieldOrdinal, boolean alias) {
        ...
        if (fieldOrdinal < 0 || fieldOrdinal > rowType.getFieldCount()) {
            throw new IllegalArgumentException(
                    "field ordinal [" + fieldOrdinal + "] out of range; input fields are: " + rowType.getFieldNames());
        }
    ...
    private GroupKey groupKey_(Iterable<? extends RexNode> nodes, boolean indicator,
            Iterable<? extends Iterable<? extends RexNode>> nodeLists) {
    ...
    public GroupKey groupKey(ImmutableBitSet groupSet, ImmutableList<ImmutableBitSet> groupSets) {
        return groupKey_(groupSet, false,
    ...
    public GroupKey groupKey(ImmutableBitSet groupSet, boolean indicator, ImmutableList<ImmutableBitSet> groupSets) {
        return groupKey_(groupSet, indicator,
                groupSets == null ? ImmutableList.of(groupSet) : ImmutableList.copyOf(groupSets));
    }
    ...
    @Deprecated // to be removed before 2.0
    public AggCall aggregateCall(SqlAggFunction aggFunction, boolean distinct, RexNode filter, String alias,
            RexNode... operands) {
        return aggregateCall(aggFunction, distinct, false, filter, ImmutableList.of(), alias,
                ImmutableList.copyOf(operands));
    }

    @Deprecated // to be removed before 2.0
    public AggCall aggregateCall(SqlAggFunction aggFunction, boolean distinct, boolean approximate, RexNode filter,
            String alias, RexNode... operands) {
        return aggregateCall(aggFunction, distinct, approximate, filter, ImmutableList.of(), alias,
                ImmutableList.copyOf(operands));
    }
    ...
    public AggCall aggregateCall(SqlAggFunction aggFunction, Iterable<? extends RexNode> operands) {
        return aggregateCall(aggFunction, false, false, null, ImmutableList.of(), null, ImmutableList.copyOf(operands));
    }
    ...
    protected AggCall aggregateCall(SqlAggFunction aggFunction, boolean distinct, boolean approximate, RexNode filter,
            ImmutableList<RexNode> orderKeys, String alias, ImmutableList<RexNode> operands) {
        return new AggCallImpl(aggFunction, distinct, approximate, filter, alias, operands, orderKeys);
    }
    ...
    public AggCall count(boolean distinct, String alias, RexNode... operands) {
        return aggregateCall(SqlStdOperatorTable.COUNT, distinct, false, null, ImmutableList.of(), alias,
                ImmutableList.copyOf(operands));
    }
    ...
    public AggCall sum(boolean distinct, String alias, RexNode operand) {
        return aggregateCall(SqlStdOperatorTable.SUM, distinct, false, null, ImmutableList.of(), alias,
    ...
    public AggCall avg(boolean distinct, String alias, RexNode operand) {
        return aggregateCall(SqlStdOperatorTable.AVG, distinct, false, null, ImmutableList.of(), alias,
                ImmutableList.of(operand));
    }
    ...
    public AggCall min(String alias, RexNode operand) {
        return aggregateCall(SqlStdOperatorTable.MIN, false, false, null, ImmutableList.of(), alias,
                ImmutableList.of(operand));
    }
    ...
    public AggCall max(String alias, RexNode operand) {
        return aggregateCall(SqlStdOperatorTable.MAX, false, false, null, ImmutableList.of(), alias,
                ImmutableList.of(operand));
    }
    ...
    public RelBuilder project(Iterable<? extends RexNode> nodes, Iterable<String> fieldNames, boolean force) {
        ...
        if (frame.rel instanceof Project && shouldMergeProject()) {
            ...
            final List<RexNode> newNodes = RelOptUtil.pushPastProject(nodeList, project);
        ...
        final Set<String> uniqueNameList = getTypeFactory().getTypeSystem().isSchemaCaseSensitive() ? new HashSet<>()
        ...
        final RelNode project = projectFactory.createProject(frame.rel, ImmutableList.copyOf(nodeList), fieldNameList);
        stack.pop();
    ...
    public RelBuilder aggregate(GroupKey groupKey, Iterable<AggCall> aggCalls) {
        ...
        if (groupKey_.nodeLists != null) {
            ...
            if (registrar.extraNodes.size() > sizeBefore) {
                throw new IllegalArgumentException("group sets contained expressions not in group key: "
                        + registrar.extraNodes.subList(sizeBefore, registrar.extraNodes.size()));
            }
        } else {
        ...
        for (AggCall aggCall : aggCalls) {
            ...
            if (aggCall instanceof AggCallImpl) {
                ...
                RelCollation collation = RelCollations.of(aggCall1.orderKeys.stream()
                        .map(orderKey -> collation(orderKey, RelFieldCollation.Direction.ASCENDING, null,
                                Collections.emptyList()))
                        .collect(Collectors.toList()));
                aggregateCall = AggregateCall.create(aggCall1.aggFunction, aggCall1.distinct, aggCall1.approximate,
                        args, filterArg, collation, groupSet.cardinality(), r, null, aggCall1.alias);
            } else {
        ...
        RelNode aggregate = aggregateFactory.createAggregate(r, groupKey_.indicator, groupSet, groupSets,
                aggregateCalls);
        ...
        for (int j = 0; j < aggregateCalls.size(); ++j) {
            ...
            final RelDataTypeField fieldType = new RelDataTypeFieldImpl(aggregateFields.get(i + j).getName(), i + j,
                    call.getType());
            fields.add(new Field(ImmutableSet.of(), fieldType));
        }
    ...
    public RelBuilder join(JoinRelType joinType, RexNode condition, Set<CorrelationId> variablesSet) {
        ...
        if (correlate) {
            ...
            join = correlateFactory.createCorrelate(left.rel, right.rel, id, requiredColumns,
    ...
    public RelBuilder join(JoinRelType joinType, String... fieldNames) {
        ...
        for (String fieldName : fieldNames) {
            conditions.add(call(SqlStdOperatorTable.EQUALS, field(2, 0, fieldName), field(2, 1, fieldName)));
        }
    ...
    public RelBuilder semiJoin(Iterable<? extends RexNode> conditions) {
        ...
        final RelNode semiJoin = semiJoinFactory.createSemiJoin(peek(), right.rel, and(conditions));
        replaceTop(semiJoin);
        return this;
    }
    ...
    public RelBuilder values(String[] fieldNames, Object... values) {
        if (fieldNames == null || fieldNames.length == 0 || values.length % fieldNames.length != 0
        ...
        for (Ord<String> fieldName : Ord.zip(fieldNames)) {
            if (allNull(values, fieldName.i, fieldNames.length)) {
                throw new IllegalArgumentException(
                        "All values of field '" + fieldName.e + "' are null; cannot deduce type");
            }
        }
    ...
    public RelBuilder sortExchange(RelDistribution distribution, RelCollation collation) {
        RelNode exchange = sortExchangeFactory.createSortExchange(peek(), distribution, collation);
        replaceTop(exchange);
        return this;
    }
    ...
    public RelBuilder sortLimit(int offset, int fetch, Iterable<? extends RexNode> nodes) {
        ...
        for (RexNode node : nodes) {
            final RelFieldCollation collation = collation(node, RelFieldCollation.Direction.ASCENDING, null,
                    extraNodes);
        ...
        if (fieldCollations.isEmpty()) {
            ...
            if (top instanceof Project) {
                ...
                if (project.getInput() instanceof Sort) {
                    ...
                    if (sort2.offset == null && sort2.fetch == null) {
                        final RelNode sort = sortFactory.createSort(sort2.getInput(), sort2.collation, offsetNode,
                                fetchNode);
    ...
    public RelBuilder match(RexNode pattern, boolean strictStart, boolean strictEnd,
            Map<String, RexNode> patternDefinitions, Iterable<? extends RexNode> measureList, RexNode after,
            Map<String, ? extends SortedSet<String>> subsets, boolean allRows,
            Iterable<? extends RexNode> partitionKeys, Iterable<? extends RexNode> orderKeys, RexNode interval) {
        final List<RelFieldCollation> fieldCollations = new ArrayList<>();
        ...
        final RelNode match = matchFactory.createMatch(peek(), pattern, typeBuilder.build(), strictStart, strictEnd,
                patternDefinitions, measures.build(), after, subsets, allRows, ImmutableList.copyOf(partitionKeys),
                RelCollations.of(fieldCollations), interval);
    ...
    protected static class GroupKeyImpl implements GroupKey {
        ...
        GroupKeyImpl(ImmutableList<RexNode> nodes, boolean indicator, ImmutableList<ImmutableList<RexNode>> nodeLists,
                String alias) {
        ...
        public GroupKey alias(String alias) {
            return Objects.equals(this.alias, alias) ? this : new GroupKeyImpl(nodes, indicator, nodeLists, alias);
        }
    }
