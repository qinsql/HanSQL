 ...
 * {@link org.lealone.hansql.optimizer.rel.core.Calc} with multiple {@link RexCall}
...
public abstract class CalcRelSplitter {
    ...
    RelNode execute() {
        ...
        for (int level = 0; level < levelCount; level++) {
            ...
            RexProgram program1 = createProgramForLevel(level, levelCount, rel.getRowType(), exprs, exprLevels,
                    inputExprOrdinals, projectExprOrdinals, conditionExprOrdinal, outputRowType);
    ...
    private int chooseLevels(final RexNode[] exprs, int conditionOrdinal, int[] exprLevels, int[] levelTypeOrdinals) {
                ...
                if (level >= levelCount) {
                    ...
                    for (int relTypeOrdinal = 0; relTypeOrdinal < relTypes.length; relTypeOrdinal++) {
                        ...
                        if (relTypes[relTypeOrdinal].canImplement(expr, condition)) {
                            ...
                            assert (level == 0) || (levelTypeOrdinals[level
                                    - 1] != levelTypeOrdinals[level]) : "successive levels of same type";
        ...
        if (levelCount > 0) {
            ...
            // implemented.
            assert "CalcRelType"
                    .equals(relTypes[0].name) : "The first RelType should be CalcRelType for proper RexLiteral"
     ...
     */
    private RexProgram createProgramForLevel(int level, int levelCount, RelDataType inputRowType, RexNode[] allExprs,
            int[] exprLevels, int[] inputExprOrdinals, final int[] projectExprOrdinals, int conditionExprOrdinal,
        ...
        final RexShuttle shuttle = new InputToCommonExprConverter(exprInverseOrdinals, exprLevels, level,
                inputExprOrdinals, allExprs);
        for (int i = 0; i < allExprs.length; i++) {
            if (exprLevels[i] == level
                    || exprLevels[i] == -1 && level == (levelCount - 1) && allExprs[i] instanceof RexLiteral) {
    ...
    public abstract static class RelType {
        ...
        protected RelNode makeRel(RelOptCluster cluster, RelTraitSet traitSet, RelBuilder relBuilder, RelNode input,
                RexProgram program) {
    ...
    private static class InputToCommonExprConverter extends RexShuttle {
        ...
        InputToCommonExprConverter(int[] exprInverseOrdinals, int[] exprLevels, int level, int[] inputExprOrdinals,
                RexNode[] allExprs) {
