...
import org.lealone.hansql.optimizer.sql.fun.SqlSingleValueAggFunction;
...
import org.lealone.hansql.optimizer.util.ReflectUtil;
import org.lealone.hansql.optimizer.util.ReflectiveVisitor;
import org.lealone.hansql.optimizer.util.Util;
import org.lealone.hansql.optimizer.util.function.Function2;
...
public class RelDecorrelator implements ReflectiveVisitor {
    ...
    private final ReflectUtil.MethodDispatcher<Frame> dispatcher = ReflectUtil.createMethodDispatcher(Frame.class, this,
            "decorrelateRel", RelNode.class);
     ...
     * {@link org.lealone.hansql.optimizer.rel.logical.LogicalCorrelate} instances removed
     */
    ...
    public Frame decorrelateRel(LogicalAggregate rel) {
        ...
        RelNode newProject = relBuilder.push(newInput).projectNamed(Pair.left(projects), Pair.right(projects), true)
                .build();
     ...
     */
    private RelNode createValueGenerator(Iterable<CorRef> correlations, int valueGenFieldOffset,
        ...
        for (CorRef corVar : correlations) {
            ...
            if (!joinedInputs.contains(newInput)) {
                ...
                if (r == null) {
                    ...
                    r = LogicalJoin.create(r, distinct, cluster.getRexBuilder().makeLiteral(true), ImmutableSet.of(),
                            JoinRelType.INNER);
                }
            }
        }
        ...
        for (CorRef corRef : correlations) {
            ...
            // each newInput.
            final int newOutput = newLocalOutputs.indexOf(newLocalOutput) + mapNewInputToNewOffset.get(newInput)
                    + valueGenFieldOffset;
    ...
    private Frame decorrelateInputWithValueGenerator(RelNode rel, Frame frame) {
        ...
        RelNode join = LogicalJoin.create(frame.r, valueGen, relBuilder.literal(true), ImmutableSet.of(),
                JoinRelType.INNER);
     ...
     * and if found, throws a {@link org.lealone.hansql.optimizer.util.Util.FoundOne}. */
    ...
    public Frame decorrelateRel(LogicalCorrelate rel) {
        ...
        for (Map.Entry<CorDef, Integer> rightOutput : new ArrayList<>(corDefOutputs.entrySet())) {
            ...
            conditions.add(relBuilder.call(SqlStdOperatorTable.EQUALS, RexInputRef.of(newLeftPos, newLeftOutput),
                    RexBuilder.getRexFactory().makeInputRef(newLeftFieldCount + newRightPos,
        ...
        RelNode newJoin = LogicalJoin.create(leftFrame.r, rightFrame.r, condition, ImmutableSet.of(),
    ...
    public Frame decorrelateRel(LogicalJoin rel) {
        ...
        final RelNode newJoin = LogicalJoin.create(leftFrame.r, rightFrame.r,
                decorrelateExpr(currentRel, map, cm, rel.getCondition()), ImmutableSet.of(), rel.getJoinType());
    ...
    private static RexInputRef getNewForOldInputRef(RelNode currentRel, Map<RelNode, Frame> map,
        ...
        return RexBuilder.getRexFactory().makeInputRef(newOrdinal,
     ...
     */
    private boolean checkCorVars(LogicalCorrelate correlate, LogicalProject project, LogicalFilter filter,
    ...
    private RelNode createProjectWithAdditionalExprs(RelNode input, List<Pair<RexNode, String>> additionalExprs) {
        ...
        for (Ord<RelDataTypeField> field : Ord.zip(fieldList)) {
            projects.add(Pair.of((RexNode) relBuilder.getRexBuilder().makeInputRef(field.e.getType(), field.i),
                    field.e.getName()));
        }
        projects.addAll(additionalExprs);
        return relBuilder.push(input).projectNamed(Pair.left(projects), Pair.right(projects), true).build();
    }
     ...
     * after decorrelation. */
    Frame register(RelNode rel, RelNode newRel, Map<Integer, Integer> oldToNewOutputs,
            SortedMap<CorDef, Integer> corDefOutputs) {
    ...
    private class RemoveCorrelationRexShuttle extends RexShuttle {
        ...
        public RexNode visitFieldAccess(RexFieldAccess fieldAccess) {
            if (cm.mapFieldAccessToCorRef.containsKey(fieldAccess)) {
                ...
                RexNode newRexNode = RexBuilder.getRexFactory().makeInputRef(corVar.field, fieldAccess.getType());
    ...
    private final class RemoveSingleAggregateRule extends RelOptRule {
        RemoveSingleAggregateRule(RelBuilderFactory relBuilderFactory) {
            super(operand(LogicalAggregate.class,
                    operand(LogicalProject.class, operand(LogicalAggregate.class, any()))), relBuilderFactory, null);
        }
    ...
    private final class RemoveCorrelationForScalarProjectRule extends RelOptRule {
        RemoveCorrelationForScalarProjectRule(RelBuilderFactory relBuilderFactory) {
            super(operand(LogicalCorrelate.class, operand(RelNode.class, any()),
                    operand(LogicalAggregate.class, operand(LogicalProject.class, operand(RelNode.class, any())))),
        ...
        public void onMatch(RelOptRuleCall call) {
            ...
            if ((!aggregate.getGroupSet().isEmpty()) || (aggregate.getAggCallList().size() != 1)
                    || !(aggregate.getAggCallList().get(0).getAggregation() instanceof SqlSingleValueAggFunction)) {
                return;
            }
            ...
            if ((right instanceof LogicalFilter) && cm.mapRefRelToCorRef.containsKey(right)) {
                ...
                right = createProjectWithAdditionalExprs(right,
                        ImmutableList.of(Pair.of(relBuilder.literal(true), "nullIndicator")));
        ...
        public void onMatch(RelOptRuleCall call) {
            ...
            if ((right instanceof LogicalFilter) && cm.mapRefRelToCorRef.containsKey(right)) {
                ...
                if (!RelMdUtil.areColumnsDefinitelyUniqueWhenNullsFiltered(mq, left, correlatedInputRefJoinKeys)) {
                    SQL2REL_LOGGER.debug("{} are not unique keys for {}", correlatedJoinKeys.toString(),
                            left.toString());
                    return;
                }
            ...
            RexInputRef nullIndicator = RexBuilder.getRexFactory().makeInputRef(nullIndicatorPos,
                    cluster.getTypeFactory().createTypeWithNullability(
                            join.getRowType().getFieldList().get(nullIndicatorPos).getType(), true));
            ...
            for (RexNode aggInputProjExpr : aggInputProjects) {
                joinOutputProjects
                        .add(removeCorrelationExpr(aggInputProjExpr, joinType.generatesNullsOnRight(), nullIndicator));
            }

            joinOutputProjects.add(rexBuilder.makeInputRef(join, nullIndicatorPos));

            final RelNode joinOutputProject = builder.push(join).project(joinOutputProjects).build();
            ...
            for (AggregateCall aggCall : aggCalls) {
                ...
                newAggCalls.add(
                        aggCall.adaptTo(joinOutputProject, argList, filterArg, aggregate.getGroupCount(), groupCount));
            }
            ...
            newAggOutputProjectList.add(rexBuilder.makeCast(
    ...
    static class CorRef implements Comparable<CorRef> {
        ...
        public boolean equals(Object o) {
            return this == o || o instanceof CorRef && uniqueKey == ((CorRef) o).uniqueKey && corr == ((CorRef) o).corr
    ...
    static class CorDef implements Comparable<CorDef> {
        ...
        @Override
        public boolean equals(Object o) {
            return this == o || o instanceof CorDef && corr == ((CorDef) o).corr && field == ((CorDef) o).field;
        }
    ...
    private static class CorelMap {
        ...
        private CorelMap(Multimap<RelNode, CorRef> mapRefRelToCorRef, SortedMap<CorrelationId, RelNode> mapCorToCorRel,
        ...
        public boolean equals(Object obj) {
            return obj == this
                    || obj instanceof CorelMap && mapRefRelToCorRef.equals(((CorelMap) obj).mapRefRelToCorRef)
    ...
    /** Builds a {@link org.lealone.hansql.optimizer.sql2rel.RelDecorrelator.CorelMap}. */
    private static class CorelMapBuilder extends RelShuttleImpl {
        ...
        final SortedSetMultimap<RelNode, CorRef> mapRefRelToCorRef = MultimapBuilder.SortedSetMultimapBuilder.hashKeys()
                .treeSetValues().build();
