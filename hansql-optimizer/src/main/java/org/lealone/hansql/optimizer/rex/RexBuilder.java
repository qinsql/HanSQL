...
import org.lealone.hansql.optimizer.rex.impl.RexFactoryImpl;
import org.lealone.hansql.optimizer.sql.SqlAggFunction;
import org.lealone.hansql.optimizer.sql.SqlCollation;
...
import org.lealone.hansql.optimizer.sql.type.ArraySqlType;
import org.lealone.hansql.optimizer.sql.type.MapSqlType;
...
public class RexBuilder {
  ...
  private static RexFactory rexFactory = new RexFactoryImpl();
  
  public static void setRexFactory(RexFactory rexFactory) {
      RexBuilder.rexFactory = rexFactory;
  }
  
  public static RexFactory getRexFactory() {
      return RexBuilder.rexFactory;
  }
  /**
  ...
  /** Creates a list of {@link org.lealone.hansql.optimizer.rex.RexInputRef} expressions,
  ...
  public List<? extends RexNode> identityProjects(final RelDataType rowType) {
    return Lists.transform(rowType.getFieldList(),
        input -> rexFactory.makeInputRef(input.getIndex(), input.getType()));
  }
  ...
  public RexNode makeCall(
      ...
      SqlOperator op,
      List<RexNode> exprs) {
    return rexFactory.makeCall(returnType, op, exprs);
  }
   ...
   * {@link #makeCall(org.lealone.hansql.optimizer.rel.type.RelDataType, org.lealone.hansql.optimizer.sql.SqlOperator, java.util.List)}
   * is preferred.</p>
   */
  public RexNode makeCall(
      SqlOperator op,
    ...
    return rexFactory.makeCall(type, op, exprs);
  }
  ...
  public RexNode makeOver(
    ...
    final RexWindow window =
        makeWindow(
            ...
            upperBound,
            physical);
    final RexOver over = rexFactory.makeOver(type, operator, exprs, window, distinct);
    RexNode result = over;
    ...
    if (nullWhenCountZero) {
      ...
      result = makeCall(
          ...
          ensureType(type, // SUM0 is non-nullable, thus need a cast
              rexFactory.makeOver(typeFactory.createTypeWithNullability(type, false),
  ...
  public RexNode makeCorrel(
      RelDataType type,
      CorrelationId id) {
    return rexFactory.makeCorrel(id, type);
  }
  ...
  public RexNode makeRangeReference(RelNode input) {
    return rexFactory.makeRangeReference(input.getRowType(), 0);
  }
  ...
  protected RexLiteral makeLiteral(
    ...
    return rexFactory.makeLiteral(o, type, typeName);
  }
  ...
  public RexDynamicParam makeDynamicParam(
      RelDataType type,
      int index) {
    return rexFactory.makeDynamicParam(type, index);
  }
  ...
  public RexNode makeLiteral(Object value, RelDataType type,
    ...
    case MULTISET:
      ...
      if (allowCast) {
        ...
        return rexFactory.makeLiteral((Comparable) FlatLists.of(operands), type,
            type.getSqlTypeName());
      }
    case ROW:
      operands = new ArrayList<>();
   ...
   * {@link org.lealone.hansql.optimizer.rex.RexLiteral#valueMatchesType}. */
