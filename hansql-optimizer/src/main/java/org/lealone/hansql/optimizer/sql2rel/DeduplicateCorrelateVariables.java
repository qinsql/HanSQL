...
public class DeduplicateCorrelateVariables extends RelHomogeneousShuttle {
    ...
    private DeduplicateCorrelateVariables(RexBuilder builder, CorrelationId canonicalId,
            ImmutableSet<CorrelationId> alternateIds) {
    ...
    private static class DeduplicateCorrelateVariablesShuttle extends RexShuttle {
        ...
        public RexNode visitCorrelVariable(RexCorrelVariable variable) {
            if (!alternateIds.contains(variable.getCorrelationId())) {
                return variable;
            }
        ...
        public RexNode visitSubQuery(RexSubQuery subQuery) {
            if (shuttle != null) {
                RelNode r = subQuery.getRel().accept(shuttle); // look inside sub-queries
