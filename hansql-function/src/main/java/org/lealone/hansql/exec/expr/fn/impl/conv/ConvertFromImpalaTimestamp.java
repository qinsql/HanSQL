...
public class ConvertFromImpalaTimestamp {
  ...
  public static class ImpalaTimestampConvertFrom implements DrillSimpleFunc {
    ...
    @Override
    public void eval() {
      org.lealone.hansql.exec.util.ByteBufUtil.checkBufferLength(in.buffer, in.start, in.end, 12);

      in.buffer.readerIndex(in.start);
      long nanosOfDay = in.buffer.readLong();
      int julianDay = in.buffer.readInt();
      long dateTime = (julianDay - org.lealone.hansql.exec.store.parquet.ParquetReaderUtility.JULIAN_DAY_NUMBER_FOR_UNIX_EPOCH) *
          org.joda.time.DateTimeConstants.MILLIS_PER_DAY + (nanosOfDay / org.lealone.hansql.exec.store.parquet.ParquetReaderUtility.NanoTimeUtils.NANOS_PER_MILLISECOND);
