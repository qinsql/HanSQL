...
import org.apache.drill.exec.context.options.OptionList;
...
public class QueryWorkUnit {
    ...
    public static class MinorFragmentDefn {
        ...
        public PlanFragment applyPlan(PhysicalPlanReader reader) throws SqlExecutorSetupException {
            // get plan as JSON
            try {
                final String plan = reader.writeJson(root);
                final String optionsData = reader.writeJson(options);
                return PlanFragment.newBuilder(fragment).setFragmentJson(plan).setOptionsJson(optionsData).build();
            } catch (JsonProcessingException e) {
                throw new SqlExecutorSetupException("Failure while trying to convert fragment into json.", e);
            }
        }
    }
    ...
    public void applyPlan(PhysicalPlanReader reader) throws SqlExecutorSetupException {
