...
public class SimpleRexRemap {
    ...
    private final Map<LogicalExpression, LogicalExpression> destExprMap;
    ...
    public RexNode rewriteWithMap(RexNode srcRex, Map<RexNode, LogicalExpression> mapRexToExpr) {
        ...
        // Visit through the nodes, if destExprMap has an entry to provide substitute to replace a rexNode, replace the
        // rexNode
    ...
    public static String getFullPath(PathSegment pathSeg) {
        ...
        return String.format("%s.%s", nameSeg.getPath(), getFullPath(nameSeg.getChild()));
    }
