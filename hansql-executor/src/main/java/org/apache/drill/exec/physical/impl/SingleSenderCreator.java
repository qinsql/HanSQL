...
public class SingleSenderCreator implements RootCreator<SingleSender> {
    ...
    public static class SingleSenderRootExec extends BaseRootExec {
        ...
        private static final ControlsInjector injector = ControlsInjectorFactory
                .getInjector(SingleSenderRootExec.class);
        ...
        public SingleSenderRootExec(RootFragmentContext context, RecordBatch batch, SingleSender config)
            ...
            oppositeHandle = handle.toBuilder().setMajorFragmentId(config.getOppositeMajorFragmentId())
                    .setMinorFragmentId(config.getOppositeMinorFragmentId()).build();
            // tunnel = context.getDataTunnel(config.getDestination());
        ...
        public boolean innerNext() {
            ...
            case NONE:
                ...
                final BatchSchema sendSchema = incoming.getSchema() == null ? BatchSchema.newBuilder().build()
                        : incoming.getSchema();

                final FragmentWritableBatch b2 = FragmentWritableBatch.getEmptyLastWithSchema(handle.getQueryId(),
                        handle.getMajorFragmentId(), handle.getMinorFragmentId(), recMajor,
                        oppositeHandle.getMinorFragmentId(), sendSchema);
                stats.startWait();
                try {
            ...
            case OK_NEW_SCHEMA:
            case OK:
                final FragmentWritableBatch batch = new FragmentWritableBatch(false, handle.getQueryId(),
                        handle.getMajorFragmentId(), handle.getMinorFragmentId(), recMajor,
