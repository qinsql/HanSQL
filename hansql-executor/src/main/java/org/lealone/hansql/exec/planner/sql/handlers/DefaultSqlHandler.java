...
import org.lealone.hansql.exec.planner.PlannerType;
import org.lealone.hansql.exec.planner.common.DrillRelOptUtil;
import org.lealone.hansql.exec.planner.cost.DrillDefaultRelMetadataProvider;
...
import org.lealone.hansql.exec.planner.logical.PreProcessLogicalRel;
import org.lealone.hansql.exec.planner.physical.DrillDistributionTrait;
import org.lealone.hansql.exec.planner.physical.PhysicalPlanCreator;
...
import org.lealone.hansql.exec.planner.physical.visitor.AdjustOperatorsSchemaVisitor;
import org.lealone.hansql.exec.planner.physical.visitor.ComplexToJsonPrelVisitor;
import org.lealone.hansql.exec.planner.physical.visitor.ExcessiveExchangeIdentifier;
import org.lealone.hansql.exec.planner.physical.visitor.FinalColumnReorderer;
import org.lealone.hansql.exec.planner.physical.visitor.InsertLocalExchangeVisitor;
import org.lealone.hansql.exec.planner.physical.visitor.LateralUnnestRowIDVisitor;
import org.lealone.hansql.exec.planner.physical.visitor.MemoryEstimationVisitor;
import org.lealone.hansql.exec.planner.physical.visitor.RelUniqifier;
import org.lealone.hansql.exec.planner.physical.visitor.RewriteProjectToFlatten;
import org.lealone.hansql.exec.planner.physical.visitor.RuntimeFilterVisitor;
import org.lealone.hansql.exec.planner.physical.visitor.SelectionVectorPrelVisitor;
import org.lealone.hansql.exec.planner.physical.visitor.SplitUpComplexExpressions;
import org.lealone.hansql.exec.planner.physical.visitor.StarColumnConverter;
import org.lealone.hansql.exec.planner.physical.visitor.SwapHashJoinVisitor;
import org.lealone.hansql.exec.planner.physical.visitor.TopProjectVisitor;
import org.lealone.hansql.exec.planner.sql.parser.UnsupportedOperatorsVisitor;
...
import org.lealone.hansql.optimizer.tools.RuleSet;
...
public class DefaultSqlHandler extends AbstractSqlHandler {
     ...
     */
    protected SqlNode rewrite(SqlNode node) throws RelConversionException, SqlExecutorSetupException {
        return node;
    }

    protected ConvertedRelNode validateAndConvert(SqlNode sqlNode)
            throws SqlExecutorSetupException, RelConversionException, ValidationException {
    ...
    protected DrillRel convertToRawDrel(final RelNode relNode) throws SqlUnsupportedException {
        if (context.getOptions().getOption(ExecConstants.EARLY_LIMIT0_OPT)
                && context.getPlannerSettings().isTypeInferenceEnabled() && FindLimit0Visitor.containsLimit0(relNode)) {
        ...
        try {
            ...
            final RelNode pruned = transform(PlannerType.HEP_BOTTOM_UP, PlannerPhase.DIRECTORY_PRUNING,
                    setOpTransposeNode);
            ...
            if (!context.getPlannerSettings().isHepOptEnabled()) {
                ...
                convertedRelNode = transform(PlannerType.VOLCANO, PlannerPhase.LOGICAL_PRUNE_AND_JOIN, pruned,
                        logicalTraits);
                ...
                if (context.getPlannerSettings().isHepPartitionPruningEnabled()) {
                    ...
                    final RelNode intermediateNode = transform(PlannerType.VOLCANO, PlannerPhase.LOGICAL, pruned,
                            logicalTraits);

                    // HEP Join Push Transitive Predicates
                    final RelNode transitiveClosureNode = transform(PlannerType.HEP, PlannerPhase.TRANSITIVE_CLOSURE,
                            intermediateNode);

                    // hep is enabled and hep pruning is enabled.
                    intermediateNode2 = transform(PlannerType.HEP_BOTTOM_UP, PlannerPhase.PARTITION_PRUNING,
                            transitiveClosureNode);

                } else {
                    // Only hep is enabled
                    final RelNode intermediateNode = transform(PlannerType.VOLCANO, PlannerPhase.LOGICAL_PRUNE, pruned,
                            logicalTraits);
                ...
                if (context.getPlannerSettings().isRowKeyJoinConversionEnabled()) {
                    ...
                    convertedRelNode = transform(PlannerType.HEP_BOTTOM_UP, PlannerPhase.ROWKEYJOIN_CONVERSION,
                            intermediateNode3);
                } else {
            ...
            // Convert SUM to $SUM0
            final RelNode convertedRelNodeWithSum0 = transform(PlannerType.HEP_BOTTOM_UP, PlannerPhase.SUM_CONVERSION,
                    convertedRelNode);
            ...
            if (drillRel instanceof DrillStoreRel) {
                ...
                // If the query contains a limit 0 clause, disable distributed mode since it is overkill for determining
                // schema.
    ...
    protected RelNode transform(PlannerType plannerType, PlannerPhase phase, RelNode input, RelTraitSet targetTraits,
        ...
        case HEP: {
            ...
            JaninoRelMetadataProvider relMetadataProvider = JaninoRelMetadataProvider
                    .of(DrillDefaultRelMetadataProvider.INSTANCE);
    ...
    protected Prel convertToPrel(RelNode drel, RelDataType validatedRowType)
        ...
        if (context.getPlannerSettings().isMemoryEstimationEnabled() && !MemoryEstimationVisitor
                .enoughMemory(phyRelNode, queryOptions, context.getActiveEndpoints().size())) {
        ...
        if (context.getPlannerSettings().isParquetRowGroupFilterPushdownPlanningEnabled()) {
            phyRelNode = (Prel) transform(PlannerType.HEP_BOTTOM_UP, PlannerPhase.PHYSICAL_PARTITION_PRUNING,
                    phyRelNode);
        }
         ...
         */
        phyRelNode = phyRelNode.accept(new SplitUpComplexExpressions(config.getConverter().getTypeFactory(),
                context.getDrillOperatorTable(), context.getPlannerSettings().functionImplementationRegistry), null);
        ...
        phyRelNode = phyRelNode.accept(
                new RewriteProjectToFlatten(config.getConverter().getTypeFactory(), context.getDrillOperatorTable()),
                null);
    ...
    protected Pair<SqlNode, RelDataType> validateNode(SqlNode sqlNode)
            throws ValidationException, RelConversionException, SqlExecutorSetupException {
        final SqlNode sqlNodeValidated = config.getConverter().validate(sqlNode);
        final Pair<SqlNode, RelDataType> typedSqlNode = new Pair<>(sqlNodeValidated,
                config.getConverter().getOutputType(sqlNodeValidated));
    ...
    private RelNode convertToRel(SqlNode node) {
        ...
        RelNode transformedNode = transform(PlannerType.HEP, PlannerPhase.SUBQUERY_REWRITE, convertedNode);
    ...
    private RelNode preprocessNode(RelNode rel) throws SqlUnsupportedException {
        ...
        PreProcessLogicalRel visitor = PreProcessLogicalRel.createVisitor(config.getConverter().getTypeFactory(),
                context.getDrillOperatorTable(), rel.getCluster().getRexBuilder());
        try {
    ...
    protected DrillRel addRenamedProject(DrillRel rel, RelDataType validatedRowType) {
        ...
        final List<String> fieldNames2 = SqlValidatorUtil.uniquify(validatedRowType.getFieldNames(),
        ...
        DrillProjectRel topProj = DrillProjectRel.create(rel.getCluster(), rel.getTraitSet(), rel, projections,
                newRowType);
