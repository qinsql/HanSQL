...
import org.lealone.hansql.exec.planner.physical.ProjectAllowDupPrel;
import org.lealone.hansql.exec.planner.physical.ProjectPrel;
import org.lealone.hansql.exec.planner.physical.WriterPrel;
import org.lealone.hansql.exec.planner.physical.visitor.BasePrelVisitor;
...
public class CreateTableHandler extends DefaultSqlHandler {
    ...
    public PhysicalPlan getPlan(SqlNode sqlNode)
        ...
        final RelNode newTblRelNode = SqlHandlerUtil.resolveNewTableRel(false, sqlCreateTable.getFieldNames(),
                validatedRowType, queryRelNode);

        final DrillConfig drillConfig = context.getConfig();
        final AbstractSchema drillSchema = resolveSchema(sqlCreateTable, config.getConverter().getDefaultSchema(),
                drillConfig);
        final boolean checkTableNonExistence = sqlCreateTable.checkTableNonExistence();
        final String schemaPath = drillSchema.getFullSchemaName();

        // Check table creation possibility
        if (!checkTableCreationPossibility(drillSchema, originalTableName, drillConfig, context.getSession(),
                schemaPath, checkTableNonExistence)) {
        ...
        StorageStrategy storageStrategy = sqlCreateTable.isTemporary() ? StorageStrategy.TEMPORARY
        ...
        // Original table name is stored in temporary tables cache, so it can be substituted to generated one during
        // querying.
        String newTableName = sqlCreateTable.isTemporary()
                ? context.getSession().registerTemporaryTable(drillSchema, originalTableName, drillConfig)
                : originalTableName;
    ...
    private DrillRel convertToDrel(RelNode relNode, AbstractSchema schema, String tableName,
            List<String> partitionColumns, RelDataType queryRowType, StorageStrategy storageStrategy)
        ...
        final DrillRel topPreservedNameProj = queryRowType.getFieldCount() == convertedRelNode.getRowType()
                .getFieldCount() ? addRenamedProject(convertedRelNode, queryRowType) : convertedRelNode;

        final RelTraitSet traits = convertedRelNode.getCluster().traitSet().plus(DrillRel.DRILL_LOGICAL);
        final DrillWriterRel writerRel = new DrillWriterRel(convertedRelNode.getCluster(), traits, topPreservedNameProj,
                schema.createNewTable(tableName, partitionColumns, storageStrategy));
    ...
    private class ProjectForWriterVisitor extends BasePrelVisitor<Prel, Void, RuntimeException> {
        ...
        public Prel visitWriter(WriterPrel prel, Void value) throws RuntimeException {
            ...
            if (partitionColumns.size() == 0) {
                ...
                final RexNode partionColComp = createPartitionColComparator(prel.getCluster().getRexBuilder(),
                        partitionColumnExprs);
                exprs.add(partionColComp);

                final RelDataType rowTypeWithPCComp = RexUtil.createStructType(cluster.getTypeFactory(), exprs,
                        fieldNames, null);

                final ProjectPrel projectUnderWriter = new ProjectAllowDupPrel(cluster,
                        cluster.getPlanner().emptyTraitSet().plus(Prel.DRILL_PHYSICAL), child, exprs,
                        rowTypeWithPCComp);
    ...
    private AbstractSchema resolveSchema(SqlCreateTable sqlCreateTable, SchemaPlus defaultSchema, DrillConfig config) {
        ...
        if (sqlCreateTable.isTemporary() && sqlCreateTable.getSchemaPath().size() == 0) {
            ...
            resolvedSchema = SchemaUtilites.resolveToMutableDrillSchema(defaultSchema, sqlCreateTable.getSchemaPath());
        }
     ...
     */
    private boolean checkTableCreationPossibility(AbstractSchema drillSchema, String tableName, DrillConfig config,
            UserSession userSession, String schemaPath, boolean checkTableNonExistence) {
        ...
        if (isTemporaryTable || SqlHandlerUtil.getTableFromSchema(drillSchema, tableName) != null) {
            if (checkTableNonExistence) {
                ...
                throw UserException.validationError()
                        .message("A table or view with given name [%s] already exists in schema [%s]", tableName,
                                schemaPath)
                        .build(logger);
            }
        }
