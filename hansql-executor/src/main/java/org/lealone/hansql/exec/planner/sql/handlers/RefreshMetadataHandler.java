...
import static org.lealone.hansql.exec.planner.sql.SchemaUtilites.findSchema;
...
import org.lealone.hansql.exec.planner.sql.parser.SqlRefreshMetadata;
...
public class RefreshMetadataHandler extends DefaultSqlHandler {
    ...
    private PhysicalPlan direct(boolean outcome, String message, Object... values) {
        return SqlPlanner.createDirectPlan(context, outcome, String.format(message, values));
    }

    private PhysicalPlan notSupported(String tbl) {
        return direct(false,
                "Table %s does not support metadata refresh. Support is currently limited to directory-based Parquet tables.",
                tbl);
    }
    ...
    public PhysicalPlan getPlan(SqlNode sqlNode) throws SqlExecutorSetupException {
        ...
        try {
            ...
            final ParquetReaderConfig readerConfig = ParquetReaderConfig.builder()
                    .withFormatConfig((ParquetFormatConfig) formatConfig).withOptions(context.getOptions()).build();
            ...
            return SqlPlanner.createDirectPlan(context, false, String.format("Error: %s", e.getMessage()));
        }
    }
