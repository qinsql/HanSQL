...
import org.lealone.hansql.exec.planner.logical.DrillWindowRel;
    ...
    public void onMatch(RelOptRuleCall call) {
        ...
        for (final Ord<Window.Group> w : Ord.zip(window.groups)) {
            ...
            if (windowBase.keys.isEmpty() && windowBase.orderKeys.getFieldCollations().isEmpty()) {
                DrillDistributionTrait distEmptyKeys = new DrillDistributionTrait(
                        DrillDistributionTrait.DistributionType.SINGLETON);

                traits = traits.plus(distEmptyKeys);
            } else if (windowBase.keys.size() > 0) {
                DrillDistributionTrait distOnAllKeys = new DrillDistributionTrait(
                        DrillDistributionTrait.DistributionType.HASH_DISTRIBUTED,
                ...
                DrillDistributionTrait distKeys = new DrillDistributionTrait(
                        DrillDistributionTrait.DistributionType.HASH_DISTRIBUTED,
            ...
            if (addMerge) {
                ...
                convertedInput = new SingleMergeExchangePrel(window.getCluster(), traits, convertedInput,
                        windowBase.collation());
            }
            ...
            for (Ord<Window.RexWinAggCall> aggOrd : Ord.zip(windowBase.aggCalls)) {
                ...
                for (RexNode operand : aggCall.getOperands()) {
                    if (operand instanceof RexInputRef) {
                        ...
                        if (rexInputRef.getIndex() >= startConstantsIndex) {
                            operand = RexBuilder.getRexFactory().makeInputRef(refIndex + constantShiftIndex,
                ...
                aggCall = RexBuilder.getRexFactory().makeWinAggCall((SqlAggFunction) aggCall.getOperator(),
                        aggCall.getType(), newOperandsOfWindowFunction, aggCall.getOrdinal(), aggCall.isDistinct());

                newWinAggCalls.add(RexBuilder.getRexFactory().makeWinAggCall((SqlAggFunction) aggCall.getOperator(),
                        aggCall.getType(), aggCall.getOperands(), aggOrd.i, aggCall.isDistinct()));
            }

            windowBase = new Window.Group(windowBase.keys, windowBase.isRows, windowBase.lowerBound,
                    windowBase.upperBound, windowBase.orderKeys, newWinAggCalls);

            input = new WindowPrel(window.getCluster(), window.getTraitSet().merge(traits), convertedInput,
                    window.getConstants(), rowType, windowBase);
