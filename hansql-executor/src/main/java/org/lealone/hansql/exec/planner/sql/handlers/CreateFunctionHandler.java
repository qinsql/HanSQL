...
import org.lealone.hansql.exec.exception.VersionMismatchException;
...
import org.lealone.hansql.exec.planner.sql.parser.SqlCreateFunction;
import org.lealone.hansql.exec.proto.UserBitShared.Jar;
import org.lealone.hansql.exec.proto.UserBitShared.Registry;
import org.lealone.hansql.exec.store.sys.store.DataChangeVersion;
import org.lealone.hansql.exec.util.JarUtil;
...
public class CreateFunctionHandler extends DefaultSqlHandler {
     ...
     */
    @Override
    public PhysicalPlan getPlan(SqlNode sqlNode) throws SqlExecutorSetupException, IOException {
        ...
        try {
            final String action = remoteRegistry.addToJars(jarManager.getBinaryName(),
            ...
            return SqlPlanner.createDirectPlan(context, true, String.format(
            ...
            return SqlPlanner.createDirectPlan(context, false, e.getMessage());
        } finally {
            if (inProgress) {
                ...
                for (String func : functions) {
                    if (remoteFunction.equals(func)) {
                        throw new FunctionValidationException(String.format("Found duplicated function in %s: %s",
                                remoteJar.getName(), remoteFunction));
                    }
                }
            }
        }
     ...
     */
    private void initRemoteRegistration(List<String> functions, JarManager jarManager,
        ...
        try {
            while (retryAttempts >= 0) {
                ...
                jars.add(Jar.newBuilder().setName(jarManager.getBinaryName()).addAllFunctionSignature(functions)
                        .build());
                ...
                try {
                    ...
                    logger.debug(
                            "Failed to update function registry during registration, version mismatch was detected.",
                            ex);
                    retryAttempts--;
                }
            }
            throw new DrillRuntimeException(
                    "Failed to update remote function registry. Exceeded retry attempts limit.");
        } catch (Exception e) {
            if (!copyJars) {
    ...
    private class JarManager {
        ...
        JarManager(SqlNode sqlNode, RemoteFunctionRegistry remoteRegistry) throws SqlExecutorSetupException {
        ...
        private void checkPathExistence(Path path) throws IOException {
            if (!fs.exists(path)) {
                throw new IOException(
                        String.format("File %s does not exist on file system %s", path.toUri().getPath(), fs.getUri()));
            }
        }
