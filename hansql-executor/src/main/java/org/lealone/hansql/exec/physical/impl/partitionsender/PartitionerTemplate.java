...
import org.lealone.hansql.exec.compile.sig.RuntimeOverridden;
...
import org.lealone.hansql.exec.physical.impl.common.CodeGenMemberInjector;
import org.lealone.hansql.exec.physical.impl.partitionsender.PartitionSenderRootExec.Metric;
...
public abstract class PartitionerTemplate implements Partitioner {
    ...
    @Override
    public final void setup(ExchangeFragmentContext context, RecordBatch incoming, HashPartitionSender popConfig,
            OperatorStats stats, OperatorContext oContext, ClassGenerator<?> cg, int start, int end)
        ...
        // Half the outgoing record batch size if the number of senders exceeds 1000 to reduce the total amount of
        // memory
        // allocated.
    ...
    protected OutgoingRecordBatch newOutgoingRecordBatch(OperatorStats stats, HashPartitionSender operator,
            Object tunnel, FragmentContext context, BufferAllocator allocator, int oppositeMinorFragmentId) {
        return this.injectMembers(
                new OutgoingRecordBatch(stats, operator, tunnel, context, allocator, oppositeMinorFragmentId));
    }
    ...
    public abstract void doSetup(@Named("context") FragmentContext context, @Named("incoming") RecordBatch incoming,
            @Named("outgoing") OutgoingRecordBatch[] outgoing) throws SchemaChangeException;
    ...
    public class OutgoingRecordBatch implements PartitionOutgoingBatch, VectorAccessible {
        ...
        public OutgoingRecordBatch(OperatorStats stats, HashPartitionSender operator, Object tunnel,
        ...
        public void flush(boolean schemaChanged) throws IOException {
            if (dropAll) {
                // If we are in dropAll mode, we still want to copy the data, because we can't stop copying a single
                // outgoing
                // batch with out stopping all outgoing batches. Other option is check for status of dropAll before
                // copying
                // every single record in copy method which has the overhead for every record all the time. Resetting
                // the output
                // count, reusing the same buffers and copying has overhead only for outgoing batches whose receiver has
                // terminated.

                // Reset the count to 0 and use existing buffers for exhausting input where receiver of this batch is
                // terminated
                recordCount = 0;
                return;
            }
            ...
            // 2. receiver wants to terminate (possible in case of queries involving limit clause). Even when receiver
            // wants
            ...
            // if the batch is not the last batch and the current recordCount is zero, then no need to send any
            // RecordBatches
            ...
            FragmentWritableBatch writableBatch = new FragmentWritableBatch(isLastBatch, handle.getQueryId(),
                    handle.getMajorFragmentId(), handle.getMinorFragmentId(), operator.getOppositeMajorFragmentId(),
                    oppositeMinorFragmentId, getWritableBatch());
