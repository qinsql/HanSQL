...
import org.lealone.hansql.exec.planner.sql.parser.SqlDropFunction;
...
public class DropFunctionHandler extends DefaultSqlHandler {
    ...
    public PhysicalPlan getPlan(SqlNode sqlNode) throws SqlExecutorSetupException, IOException {
        ...
        try {
            final String action = remoteFunctionRegistry.addToJars(jarName,
                    RemoteFunctionRegistry.Action.UNREGISTRATION);
            ...
            removeJarFromArea(JarUtil.getSourceName(jarName), remoteFunctionRegistry.getFs(),
                    remoteFunctionRegistry.getRegistryArea());

            return SqlPlanner.createDirectPlan(context, true,
                    String.format("The following UDFs in jar %s have been unregistered:\n%s", jarName,
                            deletedJar.getFunctionSignatureList()));
    ...
    private Jar unregister(String jarName, RemoteFunctionRegistry remoteFunctionRegistry) {
        ...
        while (retryAttempts >= 0) {
            ...
            try {
                ...
                logger.debug("Failed to update function registry during unregistration, version mismatch was detected.",
                        ex);
                retryAttempts--;
            }
        }
