...
import org.lealone.hansql.exec.exception.UnsupportedOperatorCollector;
import org.lealone.hansql.exec.planner.sql.DrillOperatorTable;
import org.lealone.hansql.exec.planner.sql.parser.DrillCalciteWrapperUtility;
import org.lealone.hansql.exec.util.ApproximateStringMatcher;
...
public class PreProcessLogicalRel extends RelShuttleImpl {
    ...
    public RelNode visit(LogicalProject project) {
                ...
                if (functionName.equalsIgnoreCase("convert_from") || functionName.equalsIgnoreCase("convert_to")) {
                    ...
                    if (nArgs == 2) {
                        if (function.getOperands().get(1) instanceof RexLiteral) {
                            try {
                                literal = ((NlsString) (((RexLiteral) function.getOperands().get(1)).getValue()))
                                        .getValue();
                    ...
                    if (newFunction == null) {
                        // we are here because we found some dummy convert function. (See DummyConvertFrom and
                        // DummyConvertTo)
                        throw getConvertFunctionException(functionName, literal);
                    }
        ...
        if (rewrite) {
            LogicalProject newProject = project.copy(project.getTraitSet(), project.getInput(0), exprList,
                    project.getRowType());
    ...
    public RelNode visit(LogicalJoin join) {
        ...
        join = join.copy(join.getTraitSet(), conditionExpr, join.getLeft(), join.getRight(), join.getJoinType(),
                join.isSemiJoinDone());
    ...
    private UserException getConvertFunctionInvalidTypeException(final RexCall function) {
        ...
        return UserException.parseError().message("Invalid type %s passed as second argument to function '%s'. "
                + "The function expects a literal argument.", typeName, functionName).build(logger);
    }
    ...
    private UserException getConvertFunctionException(final String functionName, final String typeName) {
        ...
        final UserException.Builder exceptionBuilder = UserException.unsupportedError().message(
                "%s does not support conversion %s type '%s'.", functionName, functionName.substring(8).toLowerCase(),
                typeNameToPrint);
        ...
        if (typeName.length() > 0) {
            ...
            if (bestMatch != null && bestMatch.length() > functionName.length()
                    && bestMatch.toLowerCase().startsWith("convert")) {
                final StringBuilder s = new StringBuilder("Did you mean ")
                        .append(bestMatch.substring(functionName.length())).append("?");
