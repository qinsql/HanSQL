 ...
 */
package org.lealone.hansql.exec.physical.impl.mergereceiver;
...
import org.lealone.hansql.exec.proto.BitData;
...
import org.lealone.hansql.exec.record.ExpandableHyperContainer;
...
import org.lealone.hansql.exec.record.SchemaBuilder;
...
public class MergingRecordBatch extends AbstractRecordBatch<MergingReceiverPOP> implements RecordBatch {
    ...
    public MergingRecordBatch(final ExchangeFragmentContext context, final MergingReceiverPOP config,
    ...
    public IterOutcome innerNext() {
        ...
        if (!hasRun) {
            ...
            final RawFragmentBatchProvider provider : fragProviders) {
                RawFragmentBatch rawBatch;
                // check if there is a batch in temp holder before calling getNext(), as it may have been used when
                // building schema
                ...
                if (rawBatch.getHeader().getDef().getRecordCount() != 0) {
                    ...
                    // keep reading till we get a batch with record count > 0 or we have no more batches to read i.e. we
                    // get null
                    try {
            ...
            if (createDummyBatch) {
                ...
                UserBitShared.RecordBatchDef dummyDef = UserBitShared.RecordBatchDef.newBuilder()
                        ...
                        .addAllField(createDummyFieldList(fieldList)).setRecordCount(0).build();

                // Create dummy header
                BitData.FragmentRecordBatch dummyHeader = BitData.FragmentRecordBatch.newBuilder().setIsLastBatch(true)
                        .setDef(dummyDef).build();

                for (int i = 0; i < p; i++) {
                    RawFragmentBatch rawBatch = rawBatches.get(i);
                    if (rawBatch == null || rawBatch.getHeader().getDef().getFieldCount() == 0) {
                        rawBatch = new RawFragmentBatch(dummyHeader, null);
            ...
            if (!isSameSchemaAmongBatches(batchLoaders)) {
                context.getExecutorState().fail(
                        new SchemaChangeException("Incoming batches for merging receiver have different schemas!"));
                return IterOutcome.STOP;
            }

            // create the outgoing schema and vector container, and allocate the initial batch
            final SchemaBuilder bldr = BatchSchema.newBuilder()
        ...
        while (outgoingBatchHasSpace) {
            ...
            if (node.valueIndex == batchLoaders[node.batchId].getRecordCount() - 1) {
                ...
                try {
                    ...
                    assert nextBatch != null || inputCounts[node.batchId] == outputCounts[node.batchId] : String.format(
                            "Stream %d input count: %d output count %d", node.batchId, inputCounts[node.batchId],
    ...
    private UserBitShared.SerializedField createDummyField(UserBitShared.SerializedField field) {
        UserBitShared.SerializedField.Builder newDummyFieldBuilder = UserBitShared.SerializedField.newBuilder()
                .setVarByteLength(0).setBufferLength(0).setValueCount(0).setNamePart(field.getNamePart())
    ...
    private boolean isSameSchemaAmongBatches(final RecordBatchLoader[] batchLoaders) {
        ...
        for (int i = 1; i < batchLoaders.length; i++) {
            if (!schema.equals(batchLoaders[i].getSchema())) {
                logger.error(
                        "Schemas are different. Schema 1 : " + schema + ", Schema 2: " + batchLoaders[i].getSchema());
                return false;
            }
        }
        return true;
    ...
    private MergingReceiverGeneratorBase createMerger() throws SchemaChangeException {
        ...
        try {
            final CodeGenerator<MergingReceiverGeneratorBase> cg = CodeGenerator
                    .get(MergingReceiverGeneratorBase.TEMPLATE_DEFINITION, context.getOptions());
    ...
    private final MappingSet MAIN_MAPPING = new MappingSet((String) null, null, ClassGenerator.DEFAULT_SCALAR_MAP,
            ClassGenerator.DEFAULT_SCALAR_MAP);
    private final MappingSet LEFT_MAPPING = new MappingSet("leftIndex", null, ClassGenerator.DEFAULT_SCALAR_MAP,
            ClassGenerator.DEFAULT_SCALAR_MAP);
    private final MappingSet RIGHT_MAPPING = new MappingSet("rightIndex", null, ClassGenerator.DEFAULT_SCALAR_MAP,
            ClassGenerator.DEFAULT_SCALAR_MAP);
    ...
    private void generateComparisons(final ClassGenerator<?> g, final VectorAccessible batch)
        ...
        for (final Ordering od : popConfig.getOrderings()) {
            ...
            final LogicalExpression fh = FunctionGenerationHelper.getOrderingComparator(od.nullsSortHigh(), left, right,
                    context.getFunctionRegistry());
    ...
    private boolean copyRecordToOutgoingBatch(final Node node) {
        assert outgoingPosition < OUTGOING_BATCH_SIZE : String
                .format("Outgoing position %d must be less than bath size %d", outgoingPosition, OUTGOING_BATCH_SIZE);
        assert ++outputCounts[node.batchId] <= inputCounts[node.batchId] : String.format(
                "Stream %d input count: %d output count %d", node.batchId, inputCounts[node.batchId],
                outputCounts[node.batchId]);
    ...
    public void dump() {
        logger.error(
                "MergingRecordBatch[container={}, outgoingPosition={}, incomingBatches={}, batchOffsets={}, "
