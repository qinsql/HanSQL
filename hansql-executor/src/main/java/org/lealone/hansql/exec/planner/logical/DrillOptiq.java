...
import static org.lealone.hansql.exec.planner.physical.PlannerSettings.ENABLE_DECIMAL_DATA_TYPE;
...
import org.lealone.hansql.common.expression.IfExpression.IfCondition;
...
public class DrillOptiq {
    ...
    public static LogicalExpression toDrill(DrillParseContext context, RelDataType type, RexBuilder builder,
            RexNode expr) {
    ...
    public static class RexToDrill extends RexVisitorImpl<LogicalExpression> {
            ...
            case SPECIAL:
                ...
                case CASE:
                    ...
                    for (int i = 1; i < caseArgs.size(); i = i + 2) {
                        elseExpression = IfExpression.newBuilder().setElse(elseExpression)
        ...
        private LogicalExpression doUnknown(RexNode o) {
            ...
            throw UserException.planError()
                    .message(UNSUPPORTED_REX_NODE_ERROR + "RexNode Class: %s, RexNode Digest: %s",
        ...
        private LogicalExpression getDrillCastFunctionFromOptiq(RexCall call) {
            ...
            case "VARCHAR":
            case "CHAR":
                castType = Types.required(MinorType.VARCHAR).toBuilder().setPrecision(call.getType().getPrecision())
                        .build();
                break;
            case "INTEGER":
            ...
            case "DECIMAL":
                if (!context.getPlannerSettings().getOptions()
                        .getOption(PlannerSettings.ENABLE_DECIMAL_DATA_TYPE_KEY).bool_val) {
                ...
                castType = TypeProtos.MajorType.newBuilder().setMinorType(MinorType.VARDECIMAL).setPrecision(precision)
        ...
        private LogicalExpression getDrillFunctionFromOptiqCall(RexCall call) {
            ...
            // TODO: once we have more function rewrites and a patter emerges from different rewrites, factor this out
            // in a better fashion
            ...
            case "extract": {
                ...
                default:
                    throw new UnsupportedOperationException(
                            "extract function supports the following time units: YEAR, MONTH, DAY, HOUR, MINUTE, SECOND");
                }
            }
            case "timestampdiff": {
        ...
        public LogicalExpression visitLiteral(RexLiteral literal) {
            ...
            case DECIMAL:
                if (context.getPlannerSettings().getOptions().getBoolean(ENABLE_DECIMAL_DATA_TYPE.getOptionName())) {
                    if (isLiteralNull(literal)) {
                        return new TypedNullConstant(
                                Types.withScaleAndPrecision(MinorType.VARDECIMAL, TypeProtos.DataMode.OPTIONAL,
                                        literal.getType().getScale(), literal.getType().getPrecision()));
                    }
        ...
        private TypedNullConstant createStringNullExpr(int precision) {
            return new TypedNullConstant(
                    Types.withPrecision(MinorType.VARCHAR, TypeProtos.DataMode.OPTIONAL, precision));
        }
    }
