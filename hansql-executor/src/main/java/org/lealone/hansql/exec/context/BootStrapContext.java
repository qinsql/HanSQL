...
import org.lealone.hansql.common.KerberosUtil;
...
import org.lealone.hansql.exec.metrics.DrillMetrics;
...
public class BootStrapContext implements AutoCloseable {
    ...
    public BootStrapContext(DrillConfig config, CaseInsensitiveMap<OptionDefinition> definitions,
        ...
        this.executor = new ThreadPoolExecutor(0, Integer.MAX_VALUE, 60L, TimeUnit.SECONDS,
                new SynchronousQueue<Runnable>(), new NamedThreadFactory("drill-executor-")) {
            @Override
        ...
        final int scanThreadPoolSize = MIN_SCAN_THREADPOOL_SIZE > numScanThreads ? MIN_SCAN_THREADPOOL_SIZE
                : numScanThreads;
        final int scanDecodeThreadPoolSize = (numCores + 1) / 2 > numScanDecodeThreads ? (numCores + 1) / 2
                : numScanDecodeThreads;
        this.scanExecutor = Executors.newFixedThreadPool(scanThreadPoolSize, new NamedThreadFactory("scan-"));
        this.scanDecodeExecutor = Executors.newFixedThreadPool(scanDecodeThreadPoolSize,
                new NamedThreadFactory("scan-decode-"));
    }
