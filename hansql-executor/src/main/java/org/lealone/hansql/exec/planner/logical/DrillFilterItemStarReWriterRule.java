...
import org.lealone.hansql.exec.planner.logical.FieldsReWriterUtil.DesiredField;
import org.lealone.hansql.exec.planner.logical.FieldsReWriterUtil.FieldsReWriter;
...
public class DrillFilterItemStarReWriterRule {
     ...
     */
    private static void transformFilterCall(DrillFilterRel filterRel, DrillProjectRel projectRel, DrillScanRel scanRel,
            RelOptRuleCall call) {
        ...
        if (projectRel != null) {
            ...
            for (DesiredField desiredField : itemStarFields.values()) {
                newProjects.add(RexBuilder.getRexFactory().makeInputRef(projectIndex, desiredField.getType()));
                projectIndex++;
            }

            RelDataType newProjectRowType = createNewRowType(projectRel.getCluster().getTypeFactory(),
                    projectRel.getRowType().getFieldList(), itemStarFields.keySet());

            newProject = new DrillProjectRel(projectRel.getCluster(), projectRel.getTraitSet(), newScan, newProjects,
                    newProjectRowType);
        }
        ...
        for (RelDataTypeField field : fieldList) {
            RexInputRef inputRef = RexBuilder.getRexFactory().makeInputRef(field.getIndex(), field.getType());
            newProjects.add(inputRef);
        }

        DrillProjectRel wrapper = new DrillProjectRel(filterRel.getCluster(), filterRel.getTraitSet(), newFilter,
                newProjects, filterRel.getRowType());
     ...
     */
    private static RelDataType createNewRowType(RelDataTypeFactory typeFactory, List<RelDataTypeField> originalFields,
    ...
    private static DrillScanRel createNewScan(DrillScanRel scanRel, Map<String, DesiredField> itemStarFields) {
        RelDataType newScanRowType = createNewRowType(scanRel.getCluster().getTypeFactory(),
                scanRel.getRowType().getFieldList(), itemStarFields.keySet());
        ...
        return new DrillScanRel(scanRel.getCluster(), scanRel.getTraitSet().plus(DrillRel.DRILL_LOGICAL),
                scanRel.getTable(), newScanRowType, columns);
    }
