...
public class SqlHandlerUtil {
    ...
    public static RelNode resolveNewTableRel(boolean isNewTableView, List<String> tableFieldNames,
        ...
        if (tableFieldNames.size() > 0) {
            ...
            if (tableFieldNames.size() != validatedRowtype.getFieldCount()) {
                ...
                throw UserException.validationError()
                        .message("%s's field list and the %s's query field list have different counts.", tblType,
                                tblType)
                        .build(logger);
            }
        ...
        // As the column names of the view are derived from SELECT query, make sure the query has no duplicate column
        // names
    ...
    public static RelNode qualifyPartitionCol(RelNode input, List<String> partitionColumns) {
        ...
        for (final String col : partitionColumns) {
            ...
            if (field == null) {
                ...
                if (SchemaPath.DYNAMIC_STAR.equals(field.getName())) {
                    ...
                    operands.add(RexBuilder.getRexFactory().makeInputRef(field.getIndex(), field.getType()));
        ...
        if (colRefStarExprs.isEmpty()) {
            ...
            return DrillRelFactories.LOGICAL_BUILDER.create(input.getCluster(), null).push(input)
                    .projectNamed(refs, names, true).build();
        }
    }
    ...
    public static Table getTableFromSchema(AbstractSchema drillSchema, String tblName) {
        try {
            ...
            throw new DrillRuntimeException(
                    String.format(
                            "Failure while trying to check if a table or view with given name [%s] already exists "
                                    + "in schema [%s]: %s",
                            tblName, drillSchema.getFullSchemaName(), e.getMessage()),
                    e);
        }
    }
     ...
     * If table exists, throws {@link org.lealone.hansql.common.exceptions.UserException} since drop was unsuccessful,
     ...
     * If view exists, throws {@link org.lealone.hansql.common.exceptions.UserException} since drop was unsuccessful,
     * otherwise assumes that other user had dropped the view and exists without error.
     *
     * @param drillSchema drill schema
