...
import org.lealone.hansql.exec.planner.sql.parser.SqlShowFiles;
import org.lealone.hansql.exec.store.AbstractSchema;
import org.lealone.hansql.exec.store.dfs.WorkspaceSchemaFactory.WorkspaceSchema;
import org.lealone.hansql.exec.store.ischema.Records;
...
public class ShowFilesHandler extends DefaultSqlHandler {
    ...
    public PhysicalPlan getPlan(SqlNode sqlNode) throws SqlExecutorSetupException {
        ...
        if (from != null) {
            ...
            if (drillSchema == null) {
                // Entire from clause is not a schema, try to obtain the schema without the last part of the specified
                // clause.
        ...
        Path endPath = fromDir == null ? new Path(wsSchema.getDefaultLocation())
                : new Path(wsSchema.getDefaultLocation(), fromDir);
        ...
        List<ShowFilesCommandResult> records = FileSystemUtil.listAllSafe(wsSchema.getFS(), path, false).stream()
                ...
                .map(fileStatus -> new ShowFilesCommandResult(
                        new Records.File(wsSchema.getFullSchemaName(), wsSchema, fileStatus)))
                .collect(Collectors.toList());

        return SqlPlanner.createDirectPlan(context.getCurrentEndpoint(), records, ShowFilesCommandResult.class);
    }
